...
flags.DEFINE_integer(
    "black_num_res_blocks", 11, "Number of residual blocks in the neural network."
)
flags.DEFINE_integer(
    "black_num_filters",
    64,
...
flags.DEFINE_string(
    "black_ckpt",
    "./checkpoints/go/9x9_12b64/training_steps_190000.ckpt",
    "Load the checkpoint file for black player.",
)

# 11b128 version
flags.DEFINE_integer(
    "white_num_res_blocks", 10, "Number of residual blocks in the neural network."
)
flags.DEFINE_integer(
    "white_num_filters",
    128,
...
flags.DEFINE_string(
    "white_ckpt",
    "./checkpoints/go/9x9_11b128/training_steps_139000.ckpt",
...
flags.DEFINE_integer(
    "num_processes", 16, "Run the games using multiple child processes"
)
...
def main():
    ...
    black_network = AlphaZeroNet(
        input_shape,
        num_actions,
        FLAGS.black_num_res_blocks,
        FLAGS.black_num_filters,
        FLAGS.black_num_fc_units,
    )
    white_network = AlphaZeroNet(
        input_shape,
        num_actions,
        FLAGS.white_num_res_blocks,
        FLAGS.white_num_filters,
        FLAGS.white_num_fc_units,
    )
    ...
    logger.info(
        f"Starting to play {FLAGS.num_games} games, this will take some time..."
    )
    ...
    writer.close()
    logger.info(
        f"Total games {FLAGS.num_games}, black won {black_won}, white won {white_won}"
    )
