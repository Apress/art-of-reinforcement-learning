...
class GomokuEnv(BoardGameEnv):
    ...
    def __init__(
        self, board_size: int = 15, num_to_win: int = 5, num_stack: int = 8
    ) -> None:
        """
        Args:
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, dict]:
        ...
        if (
            action is not None
            and action != self.resign_move
            and not 0 <= int(action) <= self.action_space.n - 1
        ):
            raise ValueError(f"Invalid action. The action {action} is out of bound.")
        if (
            action is not None
            and action != self.resign_move
            and self.legal_actions[int(action)] != 1
        ):
    ...
    def is_current_player_won(self) -> bool:
        ...
        if (
            count_sequence_length_on_dir(board, x_last, y_last, color, vertical_dirs)
            >= self.num_to_win
        ):
            return True
        ...
        if (
            count_sequence_length_on_dir(board, x_last, y_last, color, horizontal_dirs)
            >= self.num_to_win
        ):
            return True
        ...
        if (
            count_sequence_length_on_dir(board, x_last, y_last, color, diagonal_dirs_1)
            >= self.num_to_win
        ):
            return True
        ...
        if (
            count_sequence_length_on_dir(board, x_last, y_last, color, diagonal_dirs_2)
            >= self.num_to_win
        ):
            return True
...
def count_sequence_length_on_dir(
    ...
    if not isinstance(dirs, tuple) or len(dirs) > 2:
        raise ValueError(
            "Expect input arguments dirs to be tuple, and max length to be 2."
        )

    c = sum(
        [
            count_same_color_stones(board, x_start, y_start, color, d_x, d_y)
            for d_x, d_y in dirs
        ]
    )
