...
class BoardGameEnv(gym.Env):
    ...
    def __init__(
        ...
        """
        assert (
            black_player_id != white_player_id != 0
        ), "player ids can not be the same, and can not be zero"
        ...
        self.black_player = (
            black_player_id  # black player id as well as stone color on the board
        )
        self.white_player = (
            white_player_id  # white player id as well as stone color on the board
        )
        ...
        self.action_dim = (
            self.board_size**2 + 1 if self.has_pass_move else self.board_size**2
        )
    ...
    def render(self, mode="terminal"):
        ...
        outfile.write(
            f"Game over: {game_over_label}, Result: {self.get_result_string()}"
        )
        outfile.write("\n")
        outfile.write(
    ...
    def add_to_history(self, player_id, move):
        if move != self.resign_move:
            self.history.append(
                PlayerMove(color=self.get_player_name_by_id(player_id), move=move)
            )
    ...
    def observation(self) -> np.ndarray:
        ...
        features = np.zeros(
            (self.num_stack * 2, self.board_size, self.board_size), dtype=np.int8
        )
