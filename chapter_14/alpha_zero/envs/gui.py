    ...
    * AlphaZero vs. AlphaZero
    ...
    def __init__(
        ...
        self.canvas = tk.Canvas(
            self.root, width=self.window_w, height=self.window_h, bg=Colors.BOARD_BG
        )
        self.canvas.pack()
        ...
        self.black_title_var = tk.StringVar(
            value=self.get_player_title(self.env.black_player)
        )
        self.white_title_var = tk.StringVar(
            value=self.get_player_title(self.env.white_player)
        )
    ...
    def initialize_board(self):
        ...
        for start, end in itertools.chain(_col_lines(), _row_lines()):
            self.board.create_line(
                start[0], start[1], end[0], end[1], fill=Colors.LINE, width=2
            )
    ...
    def initialize_panel(self):
        ...
        for i, title_var, c, last_move_var, to_move_var, info_var in zip(
            ...
            x0, y0, x1, y1 = self.get_circle_from_pos_and_size(
                pos, self.piece_size * 0.8
            )
            ...
            player_last_move.place(
                x=offset_x, y=self.padding + 120 * self.scale, anchor="nw"
            )
    ...
    def draw_dot(self, pos, color, size, tags="stone"):
        ...
        self.board.create_oval(
            x0, y0, x1, y1, fill=color, width=0, state=tk.DISABLED, tags=tags
        )
    ...
    def draw_stone(
        self, pos, color, invert_color, text, show_indicator=False, tags="stone"
    ):
        ...
        if self.show_steps:
            self.board.create_text(
                pos[0], pos[1], text=text, fill=invert_color, tags=tags
            )
    ...
    def redraw_board(self, board, history):
        ...
        for i, item in enumerate(history):
            ...
            self.draw_piece_on_board(
                item.move, i + 1, color, invt_color, i == len(history) - 1
            )
    ...
    def make_move(self, action):
        ...
        if not self.env.is_legal_move(action):
            ...
            msgbox.showwarning(
                "Warning", f"{player} move {self.env.action_to_gtp(action)} is illegal"
            )
            return
