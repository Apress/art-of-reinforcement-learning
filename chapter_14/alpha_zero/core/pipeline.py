...
def create_mcts_player(
    ...
    [BoardGameEnv, Node, float, float, bool], Tuple[int, np.ndarray, float, float, Node]
]:
    @torch.no_grad()
    def eval_position(
        state: np.ndarray,
...
def run_selfplay_actor_loop(
    ...
    if (
        save_sgf_dir is not None
        and os.path.isdir(save_sgf_dir)
        and os.path.exists(save_sgf_dir)
    ):
    ...
    while not stop_event.is_set():
        ...
        if (
            ...
            and np.random.rand() > disable_resign_ratio
        ):
        ...
        if should_save_sgf and played_games % save_sgf_interval == 0:
            ...
            sgf_file = os.path.join(
                save_sgf_dir, f"actor{rank}_{get_time_stamp(True)}.sgf"
            )
...
def play_and_record_one_game(
    ...
    while not done:  # For each step
        ...
        if (
            ...
            logger.debug(
                f"Search root value: {root_Q}, best child value: {best_child_Q}"
            )
    ...
    game_seq = [
        Transition(state=x, pi_prob=pi, value=v)
        for x, pi, v in zip(episode_states, episode_search_pis, episode_values)
    ]
...
def run_learner_loop(  # noqa: C901
    ...
    logger.info(
        f"Training sample ratio (over one checkpoint) is {training_sample_ratio:.2f}"
    )
    if training_sample_ratio > 0.25:
        logger.warning(
            f"Training sample ratio {training_sample_ratio:.2f} might be too high"
        )
    ...
    if init_resign_threshold <= -1:
        ...
        with lock:
            var_resign_threshold.value = -1
        logger.info(
            f"Resignation is disabled for first {no_resign_games} self-play games"
        )
    else:
        with lock:
    ...
    if load_ckpt is not None and os.path.exists(load_ckpt):
        ...
        training_steps = loaded_state["training_steps"]
        logger.info(
            f'Learner loaded state from checkpoint "{load_ckpt}", last training step {training_steps}'
        )
    ...
    while True:
        try:
            ...
            if (
                ...
                and replay.num_games_added % save_replay_interval == 0
            ):
                replay_file = os.path.join(ckpt_dir, f"replay_state.ckpt")
            ...
            if (
                init_resign_threshold > -1.0
                and replay.num_games_added >= no_resign_games
            ):
                if (
                ...
                if (
                    replay.num_games_added == no_resign_games
                    or replay.num_games_added % reset_fp_interval == 0
                ):
                    resign_count = last_resign_count = could_won_count = 0
                    logger.info(
                        f"Reset resignation threshold to {init_resign_threshold}"
                    )
                    with lock:
                        var_resign_threshold.value = init_resign_threshold
                # For those resignation have been disabled games, the agent may not chose to resign depending on the search results
                    ...
                    and resign_count
                    % int(games_per_ckpt * 0.5 * disable_resign_ratio * 0.5)
                    == 0
                ):
                    ...
                    current_fp_rate = (
                        0
                        if resign_count == 0
                        else round_it(could_won_count / resign_count)
                    )
                    current_threshold = var_resign_threshold.value
                    new_threshold = maybe_adjust_resign_threshold(
                        current_threshold, current_fp_rate, target_fp_rate
                    )
            ...
            if replay.num_games_added == min_games or (
                replay.num_games_added >= min_games
                and last_ckpt_games >= games_per_ckpt
            ):
                logger.debug(
                ...
                while training_steps < target_t:
                    ...
                    optimizer.zero_grad()
                    pi_loss, v_loss = compute_losses(
                        network, device, transitions, argument_data
                    )
                    loss = pi_loss + v_loss
                    loss.backward()
                    optimizer.step()
                    ...
                    # Logging statistics
                    if (
                        training_steps % log_interval == 0
                        or training_steps % ckpt_interval == 0
                    ):
                        stats = {
                ...
                ckpt_file = os.path.join(
                    ckpt_dir, f"training_steps_{training_steps}.ckpt"
                )
                torch.save(
                    {
                ...
                logger.debug(
                    f'New checkpoint for training steps {training_steps} is created at "{ckpt_file}"'
                )
...
def compute_losses(
    network, device, transitions, argumentation=False
) -> Tuple[torch.Tensor, torch.Tensor]:
    # [B, C, N, N]
    state = torch.from_numpy(transitions.state).to(
        device=device, dtype=torch.float32, non_blocking=True
    )
    # [B, num_actions]
    target_pi = torch.from_numpy(transitions.pi_prob).to(
        device=device, dtype=torch.float32, non_blocking=True
    )
    # [B, ]
    target_v = torch.from_numpy(transitions.value).to(
        device=device, dtype=torch.float32, non_blocking=True
    )

    if argumentation:
        state, target_pi, target_v = apply_random_transformation(
            state, target_pi, target_v
        )
...
def maybe_adjust_resign_threshold(
    current_v, current_rate, target_rate, min_v=-0.9999, smoothing_factor=0.5
) -> float:
    """
...
def run_evaluator_loop(
    ...
    dataloader = None
    if (
        eval_games_dir is not None
        and eval_games_dir != ""
        and os.path.exists(eval_games_dir)
    ):
...
def eval_on_pro_games(
    ...
    for states, target_pi, target_v in dataloader:
        ...
        target_indices = torch.argmax(
            target_pi, dim=1
        )  # Get the index of the actual human move
        ...
        expanded_target_indices = target_indices.unsqueeze(1).expand(
            batch_size, max(k_list)
        )
