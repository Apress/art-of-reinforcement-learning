...
def replay_sgf(
    sgf_file, num_stack, logger, skip_n=0, min_elo=2100, max_games_per_player=200
):
    ...
    if (
        board_size is None
        or board_size == ""
        or int(board_size) != int(os.environ["BOARD_SIZE"])
    ):
    ...
    if result_str is None or result_str == "" or len(result_str) < 3:
        ...
        logger.debug(
            f'Game "{sgf_file}" with result {result_str} does not have a natural winner'
        )
        return None
    ...
    # Check for end move, also exclude 'TW' and 'TB', which are territory markup
    while (
        node.next is not None
        and "TW" not in node.next.properties
        and "TB" not in node.next.properties
    ):
        next_player = None
        next_move = None
        ...
        try:
            ...
            logger.debug(
                f"Skipping game '{sgf_file}', as move {node.next.properties} at step {env.steps} is illegal"
            )
            return None
    ...
    if not re.search(r"\+T", result_str, re.IGNORECASE) and not re.search(
        r"\+R", result_str, re.IGNORECASE
    ):
        is_mismatch = False
        ...
        if is_mismatch:
            logger.debug(
                f'Game "{sgf_file}" has mismatching result, env result: {env_result_str}, SGF result: {result_str}'
            )
...
def build_eval_dataset(games_dir, num_stack, logger=None) -> TensorDataset:
    ...
    logger.info(f"Building evaluation dataset...")
    ...
    sorted_game_counts = dict(
        sorted(GAME_COUNTS.items(), key=lambda x: x[1], reverse=True)
    )
    logger.debug(f"Number of games by player: {sorted_game_counts}")

    logger.info(
        f"Finished loading {len(eval_dataset)} positions from {valid_games} games"
    )
    return eval_dataset
