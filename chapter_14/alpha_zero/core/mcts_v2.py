...
def best_child(
    ...
    if move not in node.children:
        node.children[move] = Node(
            to_play=child_to_play, num_actions=node.num_actions, move=move, parent=node
        )
...
def generate_search_policy(
    child_N: np.ndarray, temperature: float, legal_actions: np.ndarray
) -> np.ndarray:
...
def uct_search(
    ...
    if root_node is None:
        ...
        root_node = Node(
            to_play=env.to_play, num_actions=env.action_dim, parent=DummyNode()
        )
    ...
    search_pi = generate_search_policy(
        root_node.child_N, 1.0 if warm_up else 0.1, root_legal_actions
    )
