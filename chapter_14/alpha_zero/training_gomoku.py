...
flags.DEFINE_integer(
    "num_filters", 40, "Number of filters for the conv2d layers in the neural network."
)
flags.DEFINE_integer(
    "num_fc_units",
    80,
...
flags.DEFINE_integer(
    "min_games", 5000, "Collect number of self-play games before learning starts."
)
flags.DEFINE_integer(
    "games_per_ckpt",
    5000,
...
flags.DEFINE_float(
    "l2_regularization", 1e-4, "The L2 regularization parameter applied to weights."
)
...
flags.DEFINE_bool(
    "compress_data", False, "Compress state when saving in replay buffer, default off."
)
...
flags.DEFINE_integer(
    "ckpt_interval", 1000, "The frequency (in training step) to create new checkpoint."
)
flags.DEFINE_integer(
    "log_interval", 200, "The frequency (in training step) to log training statistics."
)
flags.DEFINE_string(
    "ckpt_dir", "./checkpoints/gomoku/13x13", "Path for checkpoint file."
)
flags.DEFINE_string(
    "logs_dir",
    "./logs/gomoku/13x13",
...
flags.DEFINE_string(
    "eval_games_dir", "", "Path contains evaluation games in sgf format."
)
flags.DEFINE_string(
    "save_sgf_dir",
...
flags.DEFINE_string(
    "load_ckpt", "", "Resume training by starting from last checkpoint."
)
flags.DEFINE_string(
    "load_replay", "", "Resume training by loading saved replay buffer state."
)
...
def main():
    ...
    if torch.cuda.is_available():
        ...
        if num_gpus > 1:
            ...
            # A single RTX 3090 can handle 24-30 actors
            actor_devices = [
                torch.device(f"cuda:{i % num_gpus}") for i in range(FLAGS.num_actors)
            ]
        else:
            learner_device = eval_device = torch.device("cuda")
            actor_devices = [torch.device("cuda") for i in range(FLAGS.num_actors)]
    ...
    lr_scheduler = MultiStepLR(
        optimizer, milestones=FLAGS.lr_milestones, gamma=FLAGS.lr_decay
    )
